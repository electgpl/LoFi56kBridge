/**
 * @file    SubGHzBridge_M5_v1_2_baseline_client.ino
 * @brief   Bridge IP sub-GHz sobre GFSK 250 kbps — Elecrow ThinkNode M5
 *
 * ══════════════════════════════════════════════════════════════════════════
 *  v1.2 — BASELINE FUNCIONAL (CLIENTE)
 * ══════════════════════════════════════════════════════════════════════════
 *  BASADO EN v1.1 con NAPT fix + verificación de internet
 *
 *  FIX NAPT: napt_enable() en WIFI_AP_DEF en vez de SG
 *  VERIFICADO: ping WAN 8.8.8.8 → 256ms RTT, Telegram conectado, ~22 kbps
 *
 * ══════════════════════════════════════════════════════════════════════════
 */

#define ROLE_GATEWAY   0      // 1 = GATEWAY (STA+MASTER) | 0 = CLIENTE (AP+SLAVE)
...
// ============================================================
#if ROLE_GATEWAY
  #define WIFI_STA_SSID  "YourSSID" //Not used in CLIENT ROLE
  #define WIFI_STA_PASS  "YourPASS" //Not used in CLIENT ROLE
...
static void napt_start() {
    ...
    if (e != ESP_OK)
        Serial.println("[NAPT] Revisar CONFIG_LWIP_IPV4_NAPT / IP_FORWARD en el core");
}
...
// ============================================================
// Envía periódicamente paquetes ARP o ICMP falsos para que el enlace
// muestre throughput aunque no haya dispositivo conectado al AP.
static void benchmark_send() {
    // Construye un paquete Ethernet mínimo: dstMAC + srcMAC + EtherType + payload
    // Usamos EtherType 0x0800 (IPv4) con un ping (ICMP Echo) hacia 10.0.0.1
    uint8_t pkt[64]; // mínimo Ethernet + IP + ICMP
    ...
    uint16_t ip_len = 20 + 8;  // IP hdr + ICMP hdr (sin payload)
    pkt[16] = (ip_len >> 8) & 0xFF;
    pkt[17] = ip_len & 0xFF;
    pkt[18] = 0x00; pkt[19] = 0x00; // ID
    pkt[20] = 0x00; pkt[21] = 0x00; // flags/frag
    pkt[22] = 64;              // TTL
    pkt[23] = 0x01;            // Protocol: ICMP
    // IP checksum will be filled below
    // src IP: depends on role
    if (ROLE_GATEWAY) {
        pkt[26] = 10; pkt[27] = 0; pkt[28] = 0; pkt[29] = 1;   // 10.0.0.1
        pkt[30] = 10; pkt[31] = 0; pkt[32] = 0; pkt[33] = 2;   // dst 10.0.0.2
    } else {
        pkt[26] = 10; pkt[27] = 0; pkt[28] = 0; pkt[29] = 2;   // 10.0.0.2
        pkt[30] = 10; pkt[31] = 0; pkt[32] = 0; pkt[33] = 1;   // dst 10.0.0.1
    }
    // IP checksum
    uint32_t ck = 0;
    ...
    // Enviar por el netif como si viniera de lwIP
...
static void mac_consume(const uint8_t *p, uint8_t len) {
    ...
    // FIX 1+2: Solo procesar ACK si tenemos fragmento pendiente
    // Nota: rx_synced NO se requiere aqui porque rx_last_ok=0
    // inicialmente coincide correctamente con tx_seq=0.
    if (tx_cur != NULL) {
        if (p[O_ACK] == tx_seq) {
            frag_advance();
        } else {
    ...
    // FIX 1: marcar como sincronizado al recibir el primer DATA
    if (!rx_synced) {
        rx_synced = true;
        Serial.println("[MAC] RX synced (primer DATA recibido)");
    }

    // FIX 4: resetear tx_retries al recibir DATA valido del peer
    // (el peer está vivo, no necesitamos retransmitir tan agresivamente)
    tx_retries = 0;

    uint8_t seq = p[O_SEQ];

    // FIX 3: recuperacion de desync
    if (seq == rx_expect) {
        reasm_push(p, len);
        rx_last_ok = seq; rx_expect++;
        st.frag_rx++; st.bytes_down += (len - SG_HDR_LEN);
    } else if (p[O_CTRL] & SG_F_FIRST) {
        // Llegó un FIRST con seq inesperado: resetear reensamblaje
...
static void mac_task(void *arg) {
    uint32_t bm_timer = 0;     // FIX 5: timer para benchmark
    for (;;) {
        if (do_selftest) { do_selftest = false; run_selftest(); }
        if (!running)    { vTaskDelay(pdMS_TO_TICKS(20)); continue; }

        // FIX 5: generar tráfico benchmark periódicamente
...
#if ROLE_GATEWAY
        // ─────────── MASTER: transmite y espera respuesta ───────────
        uint32_t t0 = us_now();
        notify_flush();
        ...
        if (got) {
            ...
            if (e == RADIOLIB_ERR_NONE && rl >= SG_HDR_LEN) {
                st.rssi_loc = r;
                // Si teniamos tx_cur, el ACK del peer lo confirma
            ...
            } else { st.frag_lost++; if(st.frag_lost % 10 == 1) Serial.printf("[RX] readData err=%d rl=%d\n", e, rl); }
        } else {
            st.frag_lost++;
            radio.standby();
            if (tx_cur) {
...
static void stats_reset() {
    ...
    cyc_sum = 0; cyc_n = 0;
    // FIX 1: resetear estado del protocolo
    tx_retries = 0; rx_synced = false;
    reasm_active = false; reasm_len = 0;
    rx_expect = 0; rx_last_ok = 0;
    // No resetear tx_seq para evitar colision con el peer
    // El primer fragment post-reset lleva FIRST bit
}
...
static void wifi_start() {
    ...
    WiFi.softAPConfig(IPAddress(192,168,5,1), IPAddress(192,168,5,1),
                      IPAddress(255,255,255,0), IPAddress((uint32_t)0), IPAddress("8.8.8.8"));
...
static void cmd_wan_test() {
    int sock = socket(AF_INET, SOCK_DGRAM, 0);
    if (sock < 0) { Serial.printf("[WAN] socket errno=%d\n", errno); return; }
    struct sockaddr_in local;
    local.sin_family = AF_INET;
    local.sin_addr.s_addr = esp_ip4addr_aton("192.168.5.1");
    local.sin_port = htons(0);
    if (bind(sock, (struct sockaddr*)&local, sizeof(local)) < 0) {
        close(sock); Serial.printf("[WAN] bind errno=%d\n", errno); return;
    }
    ...
    if (r < 0) { close(sock); Serial.printf("[WAN] sendto errno=%d\n", errno); return; }
    uint8_t reply[256];
    struct sockaddr_in from;
    socklen_t fl = sizeof(from);
    r = recvfrom(sock, reply, sizeof(reply), 0, (struct sockaddr*)&from, &fl);
    ...
    if (r > 0) {
        uint32_t ip = from.sin_addr.s_addr;
        Serial.printf("[WAN] respuesta %lu ms desde %d.%d.%d.%d (%d B) eth_tx %lu->%lu\n",
                      millis() - t0,
                      ip & 0xFF, (ip >> 8) & 0xFF, (ip >> 16) & 0xFF, (ip >> 24) & 0xFF,
                      r, tx_before, tx_after);
    } else {
        Serial.printf("[WAN] timeout %lu ms eth_tx %lu->%lu (diff=%lu)\n",
    ...
    while (Serial.available()) {
        ...
        if (c == '\n' || c == '\r') {
            ...
            if (strcmp(buf, "start") == 0) {
                ...
                Serial.println("[CMD] fwd deshabilitado (causa crash)");
            } else if (strcmp(buf, "ipfwd") == 0) {
                Serial.println("[FWD] NAPT handles forwarding");
...
static void buttons() {
    ...
    // FIX 5: ambos botones >3s = toggle benchmark
    if (c1 == LOW && c2 == LOW) {
...
void setup() {
    ...
    napt_start();
    // NAPT already enables IP forwarding internally
    ...
    Serial.println("[INIT] listo. B1=START/STOP  B1+B2>3s=BM  CMD:netif/route/ping/ping8/dns/wan/reboot");
}
